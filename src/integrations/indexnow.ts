...
function mapFileToUrls(file: string, root: string): string[] {
  ...
  // Market data changed — submit all market pages
    ...
    urls.push(`${SITE}/market/cleaning/`);
    ...
    if (fs.existsSync(marketFile)) {
      ...
      for (const state of data.states) {
        urls.push(`${SITE}/market/cleaning/${state.slug}/`);
      }
      for (const city of data.cities) {
        urls.push(`${SITE}/market/cleaning/${city.slug}/`);
      }
      for (const problem of data.problems) {
        urls.push(`${SITE}/market/cleaning/${problem.slug}/`);
      }
    }
  }
