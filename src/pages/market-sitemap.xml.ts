...
export const GET: APIRoute = async ({ site }) => {
  ...
  const urls: { loc: string }[] = [
    { loc: `${siteUrl}/market/` },
    { loc: `${siteUrl}/market/cleaning/` },
  ];

  // States
  for (const state of marketData.states) {
    urls.push({ loc: `${siteUrl}/market/cleaning/${state.slug}/` });
  }

  // Cities
  for (const city of marketData.cities) {
    urls.push({ loc: `${siteUrl}/market/cleaning/${city.slug}/` });
  }

  // Problems
  for (const problem of marketData.problems) {
    urls.push({ loc: `${siteUrl}/market/cleaning/${problem.slug}/` });

    // Problem x State
    for (const st of problem.byState) {
      if (st.count >= 10) {
        urls.push({ loc: `${siteUrl}/market/cleaning/${problem.slug}/${st.slug}/` });
      }
    }
  }
